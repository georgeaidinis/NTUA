			...
			if j<=n andalso j>=0 andalso i<=m andalso i>=0 then
				...
				in
					case c of #"R" =>
						...
						else if  is_cat(x) andalso y = t then update_c(Cmap.insert(arr,(i,j),conflict(c,x)),Cmap.insert(time_array, (i,j),t),cats,water,i,j-2,#"L",t,n,m,cats_num,water_num,water_prev)
					else update_c(arr,time_array,cats,water,i,j-2,#"L",t,n,m,cats_num,water_num,water_prev)
					| #"L" =>
						if  x = #"." then update_c(Cmap.insert(arr,(i,j),c),Cmap.insert(time_array, (i,j),t),pf(cats,i,j,t+1),water,i+1,j+1,#"D",t,n,m,cats_num+1,water_num,water_prev)
						else if is_cat(x) andalso y = t then update_c(Cmap.insert(arr,(i,j),conflict(c,x)),Cmap.insert(time_array, (i,j), t+1),cats,water,i+1,j+1,#"U",t,n,m,cats_num,water_num,water_prev)
						else update_c(arr,time_array,cats,water,i+1,j+1,#"U",t,n,m,cats_num,water_num,water_prev)
					| #"D" =>
						if  x = #"." then update_c(Cmap.insert(arr,(i,j),c),Cmap.insert(time_array, (i,j),t),pf(cats,i,j,t+1),water,i-2,j,#"U",t,n,m,cats_num+1,water_num,water_prev)
						else if  is_cat(x) andalso y = t then update_c(Cmap.insert(arr,(i,j),conflict(c,x)),Cmap.insert(time_array, (i,j),t),cats,water,i-2,j,#"D",t,n,m,cats_num,water_num,water_prev)
					...
					| #"U" =>
						...
						else if  is_cat(x) andalso y = t then update_cat(Cmap.insert(arr,(i,j),conflict(c,x)),Cmap.insert(time_array, (i,j),t),cats,water,t,n,m,cats_num,water_num,water_prev)
...
(*update_water(arr,time_array,cats,water,t,n,m,cats_num,water_num,water_prev)*)
			...
			let
				...
				in
					case time-t of 1 =>  update_water(arr,time_array,pf(cats,i,j,time),water,t+1,n,m,cats_num,water_num,water_prev) (*print(Int.toString(i) ^","^ Int.toString(j) ^","^ Int.toString(time)^"\n") update_water(arr,time_array,pf(cats,i,j,time),water,t,n,m,cats_num,water_num,water_prev)
*)					| _ => update_c(arr,time_array,cats,water,i,j+1,#"R",t,n,m,cats_num,water_num,water_prev)
		...
		and update_w(arr,time_array,cats,water,i:int,j:int,c:char,t,n,m,cats_num,water_num,water_prev) =
			...
			in
				case c of #"R" =>
					if j<=n andalso x = #"." then update_w(Cmap.insert(arr,(i,j),#"W"),Cmap.insert(time_array, (i,j),t),cats,pf(water,i,j,t+1),i,j-2,#"L",t,n,m,cats_num,water_num+1,water_num)
					else if j<=n andalso is_cat(x) then update_w(Cmap.insert(arr,(i,j),kill(x)),Cmap.insert(time_array, (i,j),t),cats,pf(water,i,j,t+1),i,j-2,#"L",t,n,m,cats_num-1,water_num+1,water_num)
					else update_w(arr,time_array,cats,water,i,j-2,#"L",t,n,m,cats_num,water_num,water_prev)
				| #"L" =>
					if j>=0 andalso x = #"." then update_w(Cmap.insert(arr,(i,j),#"W"),Cmap.insert(time_array, (i,j),t),cats,pf(water,i,j,t+1),i+1,j+1,#"U",t,n,m,cats_num,water_num+1,water_num)
					else if j>=0 andalso is_cat(x) then update_w(Cmap.insert(arr,(i,j),kill(x)),Cmap.insert(time_array, (i,j),t),cats,pf(water,i,j,t+1),i+1,j+1,#"U",t,n,m,cats_num-1,water_num+1,water_num)
					else update_w(arr,time_array,cats,water,i+1,j+1,#"U",t,n,m,cats_num,water_num,water_prev)
				| #"U" =>
					if i>=0 andalso x = #"." then update_w(Cmap.insert(arr,(i,j),#"W"),Cmap.insert(time_array, (i,j),t),cats,pf(water,i,j,t+1),i-2,j,#"D",t,n,m,cats_num,water_num+1,water_num)
					else if i>=0 andalso is_cat(x) then update_c(Cmap.insert(arr,(i,j),kill(x)),Cmap.insert(time_array, (i,j),t),cats,pf(water,i,j,t+1),i-2,j,#"D",t,n,m,cats_num-1,water_num+1,water_num)
					else update_w(arr,time_array,cats,water,i-2,j,#"D",t,n,m,cats_num,water_num,water_prev)
				| #"D" =>
					if i<=m andalso x = #"." then update_water(Cmap.insert(arr,(i,j),#"W"),Cmap.insert(time_array, (i,j),t),cats,pf(water,i,j,t+1),t,n,m,cats_num,water_num+1,water_num)
					else if i<=m andalso is_cat(x) then update_water(Cmap.insert(arr,(i,j),kill(x)),Cmap.insert(time_array, (i,j),t),cats,pf(water,i,j,t+1),t,n,m,cats_num-1,water_num+1,water_num)
		...
		and update_water(arr,time_array,cats,water,t,n,m,cats_num,water_num,water_prev) =
			...
			else if Queue.isEmpty(water) then update_cat(arr,time_array,cats,water,t,n,m,cats_num,water_num,water_prev)
			else 
				let
					val (i,j,time) = Queue.dequeue(water)
				in
					
					case time-t of 1 => update_cat(arr,time_array,cats, pf(water,i,j,time),t+1,n,m,cats_num,water_num,water_prev)
		...
		*)
		and answer (arr,time_array,cats,water,t,n,m,cats_num,water_num,water_prev) =
			...
			if Queue.isEmpty(cats) andalso cats_num = 0 then
					print("Cats_num = 0" ^ "hi,"^Int.toString(t-1)^","^Int.toString(cats_num)^","^Int.toString(water_num)^","^Int.toString(water_prev))
			(*print xrono, kiniseis *)
			else if Queue.isEmpty(cats) andalso water_num = water_prev then
					print("Prev = current"^ "hi,"^Int.toString(t-1)^","^Int.toString(cats_num)^","^Int.toString(water_num)^","^Int.toString(water_prev))
