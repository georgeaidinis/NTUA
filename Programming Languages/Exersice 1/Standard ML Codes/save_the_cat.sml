		...
		and find_last_pos (arr,time_array,i,j,n,m,best_i,best_j) =
			...
			in
				if i<=n-1 andalso j<m-1 then 
					if is_cat(x) orelse is_cat(revive(x)) then
					...
					if is_cat(x) orelse is_cat(revive(x)) then
						if (valOf(Cmap.find(time_array,(i,j))) > valOf(Cmap.find(time_array,(best_i,best_j)))) then "("^Int.toString(i)^","^Int.toString(j)^")"
						else "("^Int.toString(best_i)^","^Int.toString(best_j)^")"
					else "("^Int.toString(best_i)^","^Int.toString(best_j)^")"
